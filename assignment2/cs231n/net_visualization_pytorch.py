...
def compute_saliency_maps(X, y, model):
    ...
    # scores is a tensor (N,), not a scalar, so when use backward, you should appoint a #
    # tensor (C,), so the result will be (N,C). In this case C=5.
    saliency=X.grad.data  # 如果用grad.data就不用gard.zero()
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        ...
        X_fooling.grad.zero_()  # 用了grad就要用grad.zero()
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    img.grad.zero_()  # 用了grad就要用grad.zero()
