...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        ### initialization for W & b
        layer_dims = [input_dim] + hidden_dims + [num_classes]
        for i in range(self.num_layers):
          layer_input_dim = layer_dims[i]
          layer_output_dim = layer_dims[i+1]
          self.params['W'+str(i+1)] = np.random.normal(loc=0.0, scale=weight_scale, size=(layer_input_dim, layer_output_dim))
        ...
        if self.normalization in ['batchnorm','layernorm']:
          ...
          print('What do you expect from me?')
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
          ...
          if self.normalization == None:
            out, affine_relu_cache = affine_relu_forward(out, W, b)
            ...
            out, affine_bn_relu_cache = affine_bn_relu_forward(out, W, b, gamma, beta, bn_param)
            caches.append(affine_bn_relu_cache)
            ...
            out, affine_ln_relu_cache = affine_ln_relu_forward(out, W, b, gamma, beta, ln_param)
            caches.append(affine_ln_relu_cache)
        ...
        for i in range(self.num_layers-2, -1, -1):
          ...
          if self.normalization == None:
            ...
            dw += self.reg * self.params['W'+str(i+1)] ### Dont forget reg term
            grads['W'+str(i+1)] = dw
            grads['b'+str(i+1)] = db
          elif self.normalization == 'batchnorm':
            cache = caches[i]
